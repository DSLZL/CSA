...
> [!IMPORTANT]
> CSA Manager `0.1.6` 已发布到 `@dslzl/csa` 和 GitHub `v0.1.6` Release。当前正式 patched Release 是带六个平台产物的 Codex `0.151.0` p10；runtime 验收证据覆盖 Windows x64。
...
| Patched Codex CLI | [`rust-v0.151.0-native-join-p10`](https://github.com/DSLZL/CSA/releases/tag/compat-rust-v0.151.0-native-join-p10) | Windows x64/arm64、Linux x64/arm64 musl、macOS x64/arm64；验收：Windows x64 |
...
[`v0.1.6` Release](https://github.com/DSLZL/CSA/releases/tag/v0.1.6) 还提供预编译的 Manager 压缩包和 `SHA256SUMS`。
...
选择器使用一个仅供展示的小型目录，但最终选中的 Release 仍必须匹配已观测的 tag commit、descriptor、完整 checksum 清单、声明的 upstream 身份、target、size 和 SHA-256。在线安装只下载这两个元数据文件和当前平台的可执行文件；patch、source hash、test contract 与 source manifest 继续作为 Release 构建/审计资产。无需登录 GitHub：CSA 会并行运行五秒上限的 Cloudflare 与阿里系淘宝 IP 国家探测，任意一个明确返回中国大陆（`CN`）就从固定 gh-proxy 节点池中选用首个可用节点；其他地区默认直连 GitHub，并保留直连到镜像池的兜底。
...
- 在官方与 patched Codex 之间切换时保持 state database migration 兼容。
