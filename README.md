...
The picker uses a small display-only catalog, but the selected Release still has to match the observed tag commit, descriptor, complete checksum inventory, declared upstream identity, target, size, and SHA-256. Online install downloads only those two metadata files and the current-platform executable; patches, source hashes, test contracts, and source manifests remain Release build/audit assets. No GitHub login is required: five-second Cloudflare and Alibaba/Taobao country probes run in parallel, and either one reporting mainland China (`CN`) selects the first responsive host from a fixed gh-proxy node pool. Other regions start with GitHub directly and retain direct-to-pool fallback.
